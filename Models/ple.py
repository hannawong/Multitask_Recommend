...
class Config(object):
    def __init__(self,data_dir):
        ...
        self.embed_size = 10
        self.label_dict = [2,2]
        self.num_feature = COL_NUM*self.embed_size ##[todo]
        self.num_experts = 8
        self.num_tasks = 2
        self.units= 16
        self.hidden_units= 8
...
class GatingNetwork(nn.Module):
    ...
    def __init__(self,input_units,units,num_experts,selectors):
        ## input_units: 400, num_experts: 2, selectors: 8
    ...
    def forward(self,gate_output_shared_final,gate_output_task1_final,gate_output_task2_final):
        ...
        expert_shared_tensors = expert_shared_tensors.view(-1,self.num_expers,self.units)  ##[256,8,16]
        ...
        gate_output_task1 = self.gate_activation(gate_output_task1) #[256,16]
        #print(gate_output_task1.shape)
...
class Model(nn.Module):
    ...
    def forward(self,x):
        ...
        x = torch.cat([x[:,i,:] for i in range(0,self.field_size)],axis = 1)  

        gate_output_shared, gate_output_task1, gate_output_task2 = self.gate1(x,x,x) ##[256,16]
        _,task1_o,task2_o = self.gate2(gate_output_shared, gate_output_task1, gate_output_task2) ##[256,26]
