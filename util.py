...
class Dataiterater(object):
    ...
    def _to_n_tensor(self, datas):
        ...
        x =torch.LongTensor(data[:,:-2]).to(self.device)
        y1 = torch.FloatTensor(data[:,-2]).to(self.device)
        y2 = torch.FloatTensor(data[:,-1]).to(self.device)
    
        #x = torch.FloatTensor([_[0] for _ in datas]).to(self.device) ##[256,402]
        #y1 = torch.FloatTensor([_[1] for _ in datas]).to(self.device) ##[256]
        #y2 = torch.FloatTensor([_[2] for _ in datas]).to(self.device) ##[256]
